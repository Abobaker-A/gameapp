...
export default function Categories() {
  const {x} =  useParams()
  const {gamePlatforms ,getGameplatforms } = useContext(ApiContext); 
  const more = useRef();
  const [pages, setPages] = useState(20)

  function seeMore(){
    if(gamePlatforms.length<20||gamePlatforms.length<pages){
  ...
  <div className="row gy-4">
    {gamePlatforms?.slice(0,pages).map((game , idx )=><Game gameInfo={game} key={idx}/>)}
